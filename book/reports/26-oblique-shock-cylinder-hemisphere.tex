...
\end{tikzpicture}
\caption{Схематична скінченно-елементна сітка розрахункової області біля комбінованої циліндрично-сферичної оболонки.}
\end{figure}
...
Методом скінченних елементів визначено розподіл дифракційного тиску на зовнішній поверхні комбінованої оболонки. Процес поширення хвилі моделювали на часовому інтервалі $9\times10^{-4}$~с. Розрахунки показують різке зростання тиску в момент проходження фронту, подальше затухання та появу відкладених локальних максимумів у різних точках периметра.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}
\begin{axis}[
 width=0.93\textwidth,height=6.0cm,
 xlabel={$t$, с},ylabel={$p_d$, Па},
 xmin=0,xmax=9e-4,ymin=0,ymax=6e6,
 scaled x ticks=base 10:-4,
 grid=major,legend style={font=\scriptsize,at={(0.98,0.96)},anchor=north east}]
 \addplot[thick,domain=0:0.0009,samples=250] {x<0.00006 ? 0 : 5.6e6*exp(-(x-0.00006)/0.00013)};
 \addplot[dashed,thick,domain=0:0.0009,samples=250] {x<0.00008 ? 0 : 5.1e6*exp(-(x-0.00008)/0.00015)};
 \addplot[dashdotted,thick,domain=0:0.0009,samples=250] {x<0.00019 ? 0 : 3.6e6*exp(-(x-0.00019)/0.00017)};
 \addplot[dotted,thick,domain=0:0.0009,samples=250] {x<0.00028 ? 0 : 1.5e6*exp(-(x-0.00028)/0.00020)};
 \legend{передня точка,верхня дуга,бічна ділянка,тильна ділянка}
\end{axis}
\end{tikzpicture}
\caption{Схематичне відтворення характерних залежностей дифракційного тиску від часу у вибраних точках периметра.}
\end{figure}

Максимальний тиск виникає на навітряній частині оболонки й зменшується вздовж периметра до тильної ділянки. Час досягнення максимуму при цьому зростає, що відображає послідовне проходження фронту та дифракцію хвилі навколо оболонки.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[x=0.85cm,y=0.85cm,line cap=round,line join=round]
...
\end{tikzpicture}
\caption{Схематична епюра максимального дифракційного тиску вздовж периметра оболонки.}
\end{figure}
